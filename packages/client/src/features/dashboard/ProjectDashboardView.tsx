...
/** Present in the practitioner app; absent on the read-only shared view. */
export interface DashboardNav {
  projectId: string;
  onSaveWatchList: (ids: string[]) => void;
}
...
/**
 * The project dashboard body, rendered by both the in-app page (with `nav`:
 * links into the app + watch-list editing) and the token-shared read-only view
 * (no `nav`: same content, no navigation, nothing editable). One rendering so
 * a stakeholder sees exactly what the practitioner sees.
 */
export function ProjectDashboardView({ d, nav }: { d: ProjectDashboardDto; nav?: DashboardNav }) {
  ...
  const navLink = (to: string, label: string): ReactNode =>
    nav ? (
      <Link to={`/projects/${nav.projectId}${to}`} className="text-xs font-medium text-indigo-600 hover:underline">
        {label}
      </Link>
    ) : null;
  ...
  return (
    <div className="max-w-5xl space-y-4">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="cmt-card">
          <div className="mb-2 flex items-center justify-between">
            ...
            {navLink('/assessments', 'Go to assessments →')}
          </div>
          {d.pct ? (
              ...
              <ul className="space-y-1">
                {d.groupRisks.map((g) => (
                  <li key={g.groupId} className="flex items-center justify-between text-sm">
                    {nav ? (
                      <Link
                        to={`/projects/${nav.projectId}/impact/${g.groupId}`}
                        className="text-indigo-700 hover:underline"
                      >
                        {g.groupName}
                      </Link>
                    ) : (
                      <span className="text-slate-700">{g.groupName}</span>
                    )}
        ...
        <div className="cmt-card">
          <div className="mb-2 flex items-center justify-between">
            ...
            {navLink('/impact', 'View impacted groups →')}
          </div>
        ...
        <div className="cmt-card">
          <div className="mb-2 flex items-center justify-between">
            ...
            {navLink('/assessments', 'ADKAR assessments →')}
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="w-64">
                <MultiSelect
                  ...
                  onChange={(ids) => {
                    if (ids.length <= 5) nav.onSaveWatchList(ids);
                  }}
                />
              </div>
            </div>
        ...
        {watched.length === 0 ? (
          <p className="py-6 text-center text-sm text-slate-400">
            {nav
              ...
              : 'No key impacted groups selected yet.'}
          </p>
        ) : (
              ...
              {watched.map((g) => (
                <tr key={g.id}>
                  <td className="cmt-td">
                    {nav ? (
                      <Link
                        to={`/projects/${nav.projectId}/impact/${g.id}`}
      ...
      <div className="cmt-card flex items-center justify-between">
        ...
        {nav && (
          <Link to={`/projects/${nav.projectId}/cm-performance`} className="cmt-btn-secondary">
